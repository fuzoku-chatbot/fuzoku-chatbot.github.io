...
ws.onopen = e => {
	ws.send(JSON.stringify({"auth": "sender", "password": "1234"}));
}
  ...
  if ((authed == 'true') && (token['access_token'] != 'undefined')) {
    ...
    if (response.ok) {
      try {
        ...
        if (authStatus === true) {
          ws.send(JSON.stringify({'to': 'user', 'icon': userData['picture'], 'msg': `${userData['name']} : Login authorized`}));
          ...
          ws.send(JSON.stringify({'to': 'user', 'icon': userData['picture'], 'msg': `${userData['name']} : Login unauthorized`}));
          document.cookie = "auth_token=; max-age=0";
