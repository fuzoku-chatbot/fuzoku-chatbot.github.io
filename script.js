		...
		(async() => {
			...
			if ((parsedUserQuestion.searchWord == '') || (parsedUserQuestion.needKey
				...
				if (String(result).startsWith('error:') === true) {
					...
					if (result.endsWith('500') || result.endsWith('502') || result.endsWith(
          ...
          `;
						...
						for (let i = 0; i < parsedUserQuestion.needKey.length; i++) {
							if (parsedUserQuestion.needKey[i] == 'attendance') answer.unshift(
...
function parseUserQuestion(userQuestion) {
	...
	if (dateMatch) {
		...
		searchWord = formatDate(tomorrow.getFullYear(), tomorrow.getMonth(), tomorrow
			.getDate());
